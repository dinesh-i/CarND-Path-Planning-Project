		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					// We will interpolate these waypoints using spline and fill it in with more points that control the speed
					vector<double> ptsx;
					vector<double> ptsy;
					...
					else
					{
						...
						// Use 2 points that makes the path tangent to the previous path's end point
					...
					// Define the actual end point we will use for the planner
					...
					// calculate how we break up the spline points so that we travel at our desired reference velocity
					...
					for (int i = 1; i <= 50 - previous_path_x.size(); i++)
					{
						...
						double N = (target_dist / (0.02 * ref_vel / 2.24)); // Convert miles/hour to meters/sec by dividing by 2.24
